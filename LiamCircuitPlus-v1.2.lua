...
local function CircuitPlusInitializeIfNeeded(player)
    ...
    if player.CircuitPlus_LastLaps == nil or player.CircuitPlus_Realtime == nil then
...
local function CircuitPlusHUDGame(v)
    ...
    local bestTimeSkinsForMe = ""
    for skinName, playerName in pairs(bestTimesNames) do
        if playerName == displayplayer.name then
            bestTimeSkinsForMe = bestTimeSkinsForMe..skinName..' '
        end
    end

    v.drawString(16, scrH - 64 + 8, "Winning in: "..bestTimeSkinsForMe, V_PERIDOTMAP)
